...
async function embedCompanyLogo(pdf:PDFDocument,company:any){
  ...
  return await pdf.embedPng(await fetchLogoBytes('logo-avh.png'));
}
...
function drawHeader(page:PDFPage,company:any,bold:PDFFont,regular:PDFFont,logo:PDFImage,po:string,date:string){
  ...
  const details=[company?.tax_id?`RUC ${company.tax_id}`:'',company?.address||''].filter(Boolean).join(' · ');
  drawText(page,details,centerX,yy-1,regular,6.8,muted,centerW);
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const pdf=await PDFDocument.create(),regular=await pdf.embedFont(StandardFonts.Helvetica),bold=await pdf.embedFont(StandardFonts.HelveticaBold),logo=await embedCompanyLogo(pdf,company),docDate=datePY(p.po_generated_at||p.created_at);
    let page=pageBase(pdf,company,bold,regular,logo,p.po_number,docDate),y=726;
    ...
    const pdfBytes=await pdf.save(),filename=`${p.po_number}-${safe(supplier.name||'PROVEEDOR').replace(/[^A-Za-z0-9_-]+/g,'_').slice(0,40)}.pdf`,path=`${purchaseId}/${p.po_number}-professional-v3.pdf`;
