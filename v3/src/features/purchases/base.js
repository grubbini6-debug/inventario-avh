...
(function(){
  ...
  const DELIVERY_MODE={single:'Entrega única',partial:'Entregas parciales / liberaciones'};
  ...
  window.renderPurchases=function(){
    ...
    function draw(){const q=$('#purchaseSearch').value.trim().toLowerCase(),s=$('#purchaseStatus').value,c=$('#purchaseCompany').value,d=$('#purchaseDest').value,f=$('#purchaseFrom').value,t=$('#purchaseTo').value;const filtered=rows.filter(p=>{const text=[p.company_name,p.supplier_name,p.order_reference,p.destination_text,p.warehouse_name,p.requester,p.sector,p.invoice_number,...purchaseItems(p.id).map(x=>x.description)].filter(Boolean).join(' ').toLowerCase();const date=p.ordered_date||'';return(!q||text.includes(q))&&(s==='all'||p.status===s)&&(c==='all'||p.company_id===c)&&(d==='all'||p.destination_type===d)&&(!f||date>=f)&&(!t||date<=t)});$('#purchaseCount').textContent=`${filtered.length} compras`;const list=$('#purchaseList');list.innerHTML=filtered.map(p=>{const total=purchaseTotal(p),items=purchaseItems(p.id),pct=items.length?Math.min(100,Math.round(items.reduce((a,x)=>a+Number(x.received_qty||0),0)/Math.max(1,items.reduce((a,x)=>a+Number(x.quantity||0),0))*100)):0;return`<div class="row clickable purchase-row ${p.urgency}" data-purchase="${p.id}"><div class="line"><div class="grow"><div class="title">${esc(p.supplier_name||'Proveedor sin definir')} · ${esc(p.company_name||companyName(p.company_id))}</div><div class="subtext">${dateOnly(p.ordered_date)} · ${esc(PURCHASE_TYPE[p.purchase_type]||p.purchase_type)} · ${esc(purchaseDest(p))}${p.order_reference?` · Ref. ${esc(p.order_reference)}`:''}</div><div class="metric-pills"><span>${items.length} ítem${items.length===1?'':'s'}</span><span>${esc(URGENCY_LABEL[p.urgency]||p.urgency)}</span>${p.expected_date?`<span>Entrega ${dateOnly(p.expected_date)}</span>`:''}</div>${p.destination_type==='warehouse'?`<div class="purchase-progress"><i style="width:${pct}%"></i></div>`:''}</div><div style="text-align:right;display:grid;justify-items:end;gap:7px">${statusBadge(p.status)}<div class="purchase-money">${money(total,p.currency)}</div><button type="button" class="btn sm soft" data-purchase-open="${p.id}">Abrir compra</button></div></div></div>`}).join('')||'<div class="empty">No hay compras con esos filtros.</div>';list.onclick=async e=>{const explicit=e.target.closest?.('[data-purchase-open]');const row=e.target.closest?.('[data-purchase]');if(!explicit&&!row)return;if(e.target.closest?.('[data-open-po]'))return;e.preventDefault();e.stopPropagation();const id=explicit?.dataset.purchaseOpen||row?.dataset.purchase;if(!id)return;try{if(typeof window.openPurchaseDetail!=='function')throw Error('No está disponible el detalle de compra.');await window.openPurchaseDetail(id)}catch(err){console.error('No se pudo abrir la compra',err);alert('No se pudo abrir la compra. Recargá la página y probá de nuevo. Si continúa, avisá al administrador.')}}}
  ...
  function openNewPurchase(preselectedSupplierId=null,preselectedProductId=null,preselectedQty=null){
    ...
    openModal('Nueva compra','Solo administrador · sin numeración automática',`<div class="two"><div class="field"><label>Empresa que compra/paga *</label><div class="line" style="gap:6px"><select id="pcCompany" style="flex:1">${D.purchaseCompanies.filter(x=>x.active).map(x=>`<option value="${x.id}">${esc(x.name)}</option>`).join('')}</select><button id="pcAddCompany" type="button" class="btn sm soft">+ Empresa</button></div></div><div class="field"><label>Proveedor</label><div class="line" style="gap:6px"><select id="pcSupplier" style="flex:1"><option value="">Sin definir</option>${D.suppliers.map(x=>`<option value="${x.id}">${esc(x.name)}</option>`).join('')}</select><button id="pcAddSupplier" type="button" class="btn sm soft">+ Proveedor</button></div></div></div>
      ...
      <div class="two"><div class="field"><label>Forma de pago</label><input id="pcPayment" placeholder="Transferencia, crédito, contado…"></div><div class="field"><label>Condición</label><input id="pcTerms" placeholder="30 días, 50% anticipo…"></div></div><div class="field"><label>Modalidad de entrega</label><select id="pcDeliveryMode"><option value="single" selected>Entrega única</option><option value="partial">Entregas parciales / liberaciones</option></select><div class="hint">Usá parciales cuando el proveedor vaya entregando por remitos según necesidad.</div></div>
  ...
  window.openPurchaseDetail=async function(id){
    ...
    page.innerHTML=`<div class="purchase-record">
      ...
      <div class="purchase-record-panel on" data-purchase-panel="summary">
        ...
        <div class="card"><div class="section-head" style="margin-top:0"><div><h2>Compra, factura y entrega</h2><p>Lo comercial separado de lo que entró físicamente al depósito.</p></div></div><div class="purchase-summary-grid"><div class="detail-box"><span>Comprado</span><b>${money(total,p.currency)}</b><small>100% de la OC</small></div><div class="detail-box"><span>Facturado</span><b>${hasInvoice?esc(p.invoice_number||invoiceDoc?.document_number||'Factura adjunta'):'Pendiente'}</b><small>${hasInvoice?'Documento registrado':'Sin factura cargada'}</small></div><div class="detail-box"><span>Recibido</span><b>${pct}% · ${money(receivedValue,p.currency)}</b><small>Solo recepción física confirmada</small></div><div class="detail-box"><span>Pendiente de entregar</span><b>${Math.max(0,100-pct)}% · ${money(pendingValue,p.currency)}</b><small>${deliveryMode==='partial'?'Saldo por futuras liberaciones':'Saldo pendiente de recepción'}</small></div></div></div>
        ...
        <div class="card"><div class="two"><div class="field" style="margin:0"><label>Estado</label><select id="pdStatus">${Object.entries(PURCHASE_STATUS).map(([k,v])=>`<option value="${k}" ${k===p.status?'selected':''}>${v}</option>`).join('')}</select></div><div class="field" style="margin:0"><label>Factura</label><input id="pdInvoice" value="${esc(p.invoice_number||'')}" placeholder="Nº factura"></div></div><div class="two" style="margin-top:9px"><div class="field" style="margin:0"><label>Modalidad de entrega</label><select id="pdDeliveryMode"><option value="single" ${deliveryMode==='single'?'selected':''}>Entrega única</option><option value="partial" ${deliveryMode==='partial'?'selected':''}>Entregas parciales / liberaciones</option></select></div><div class="field" style="margin:0"><label>Fecha prometida</label><input id="pdExpected" type="date" value="${p.expected_date||''}"></div></div><div class="field" style="margin-top:9px"><label>Referencia / OC</label><input id="pdReference" value="${esc(p.order_reference||'')}"></div><button id="pdSave" class="btn primary" style="margin-top:10px">Guardar cambios</button><div id="pdMsg"></div></div>
      </div>
    ...
    $('#pdSave').onclick=async()=>{const b=$('#pdSave');b.disabled=true;const r=await rpc('admin_update_purchase',{p_purchase_id:id,p_patch:{status:$('#pdStatus').value,invoice_number:$('#pdInvoice').value.trim()||null,delivery_mode:$('#pdDeliveryMode').value,expected_date:$('#pdExpected').value||null,order_reference:$('#pdReference').value.trim()||null}});b.disabled=false;if(r.error)return msg($('#pdMsg'),r.error);msg($('#pdMsg'),'Compra actualizada.',true);await loadAll(true);setTimeout(()=>openPurchaseDetail(id),100)};
