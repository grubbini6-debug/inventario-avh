(function(){
window.voidMovement=async function(id){
  if(profile.role!=='admin')return alert('Solo el administrador puede anular movimientos directamente.');
  ...
  if(m.type==='correction')return alert('Las correcciones no se eliminan desde esta opción.');
  const label=(TYPE_LABEL[m.type]||m.type)+' #'+m.movement_no;
  if(!confirm(`¿Anular ${label}?\n\nNo necesitás solicitar corrección. Como administrador, esto se ejecuta directamente. El stock se revierte y el historial queda guardado para auditoría.`))return;
  const reason=prompt('Motivo:','Cargado por error');
  if(reason===null)return;
  const r=await rpc('admin_void_movement',{p_movement_id:id,p_reason:reason.trim()||'Cargado por error'});
  if(r.error)return alert(r.error);
  alert('Movimiento anulado correctamente.');
  await loadAll(true);
  try{renderModule(activeModule||'movements')}catch(e){}
};

const oldRequestCorrection=window.requestCorrection;
window.requestCorrection=function(id){
  if(profile?.role==='admin')return voidMovement(id);
  return oldRequestCorrection(id);
};
...
window.bindMovementRows=function(){
  ...
  $$('[data-void-movement]').forEach(x=>x.onclick=e=>{e.stopPropagation();voidMovement(x.dataset.voidMovement)});
};
})();
