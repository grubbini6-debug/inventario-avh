// AVH — Flujo principal de compra: presupuesto IA -> ubicación -> OC -> confirmar compra.
(function(){
  ...
  const assetUrl=p=>{try{return new URL(p,document.baseURI).href}catch{return p}};
  const st=document.createElement('style');
  st.textContent=`
    .smart-buy-drop{border:2px dashed #b8cfc0;border-radius:18px;padding:24px;text-align:center;background:#f8fbf9}.smart-buy-drop.drag{border-color:var(--green);background:#f0f8f3}.smart-buy-drop input{display:none}.smart-buy-main{display:grid;gap:12px}.smart-buy-summary{border:1px solid #dce8df;border-radius:16px;padding:14px;background:#fff}.smart-buy-total{font-size:25px;font-weight:900}.smart-buy-dest{border:2px solid #dbe9df;border-radius:16px;padding:14px;background:#fbfdfb}.smart-buy-items{overflow:auto}.smart-buy-items table{min-width:760px;width:100%;border-collapse:collapse}.smart-buy-items th,.smart-buy-items td{padding:8px;border-bottom:1px solid #e8eeea;text-align:left}.smart-buy-items input,.smart-buy-items select{min-width:95px}.po-sheet{background:#fff;border:1px solid #dce5df;border-radius:16px;padding:22px;color:#17231d}.po-head{display:flex;justify-content:space-between;gap:22px;border-bottom:2px solid #183c2c;padding-bottom:14px}.po-brand{display:flex;gap:15px;align-items:center;min-width:0}.po-logo{width:190px;max-height:82px;object-fit:contain;object-position:left center}.po-company{font-size:13px;line-height:1.45}.po-title{font-size:27px;font-weight:950;letter-spacing:.02em}.po-no{font-size:20px;font-weight:900}.po-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:14px 0}.po-box{border:1px solid #e0e8e3;border-radius:12px;padding:11px;line-height:1.45}.po-box span,.po-meta span{display:block;font-size:11px;color:#6b7c72;font-weight:700;letter-spacing:.03em}.po-meta-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:12px 0}.po-meta{border:1px solid #e0e8e3;border-radius:10px;padding:9px;min-height:55px}.po-table{width:100%;border-collapse:collapse;margin-top:14px}.po-table th,.po-table td{padding:9px 7px;border-bottom:1px solid #e2e8e4;text-align:left;vertical-align:top}.po-table th{font-size:11px;text-transform:uppercase;color:#53675c}.po-table th:nth-child(n+3),.po-table td:nth-child(n+3){text-align:right}.po-total{font-size:22px;font-weight:950;text-align:right;margin-top:14px}.po-reference{margin-top:12px;padding-top:10px;border-top:1px solid #e5ebe7;font-size:12px}.po-footer{display:flex;justify-content:space-between;gap:20px;margin-top:26px;padding-top:13px;border-top:1px solid #dfe7e2;font-size:12px}.po-footer b{display:block;margin-bottom:2px}.po-status{display:inline-block;padding:6px 10px;border-radius:999px;font-weight:800;background:#fff4d6;color:#815b00}.po-status.ok{background:#e9f7ee;color:#146b35}.smart-buy-warning{color:#8b5a00;font-size:12px;margin-top:4px}.assistant-launch{white-space:nowrap}@media(max-width:700px){.po-head,.po-footer{flex-direction:column}.po-meta-grid{grid-template-columns:1fr 1fr}.po-logo{width:150px}.po-grid{grid-template-columns:1fr}}`;
  ...
  function bargeOptions(){return `<option value="">Sin asignar</option>${(D.barges||[]).filter(x=>x.active).sort((a,b)=>Number(a.number)-Number(b.number)).map(b=>`<option value="${b.id}">Barcaza ${b.number}</option>`).join('')}`}
  ...
  function openSmartPurchase(){
    ...
    openModal('Nueva compra','Presupuesto → ubicación → OC → confirmar compra',`<div class="smart-buy-main">
  ...
  function renderDetected(doc){
    ...
    $('#smartResult').innerHTML=`<div class="smart-buy-summary"><div class="line"><div><div class="title">${safe(doc.supplier_name||'Proveedor detectado')}</div><div class="subtext">${safe(doc.document_number?'Presupuesto '+doc.document_number:'Presupuesto')} · ${safe(doc.payment_terms||'Condición de pago no indicada')} ${conf?`· confianza ${conf}%`:''}</div></div><div class="smart-buy-total">${money(total,doc.currency)}</div></div></div>
      <div class="smart-buy-dest"><div class="title">¿Dónde se entrega?</div><div class="two" style="margin-top:8px"><div class="field"><label>Ubicación *</label><select id="smartLocation">${warehouseOptions()}<option value="other">Otra ubicación…</option></select></div><div class="field hide" id="smartOtherWrap"><label>Otra ubicación</label><input id="smartDestText" placeholder="Escribí el lugar de entrega"></div><div class="field"><label>Barcaza / proyecto</label><select id="smartBarge">${bargeOptions()}</select><div class="hint">Opcional. Sirve para trazabilidad de materiales.</div></div></div></div>
    ...
    const location=$('#smartLocation');function sync(){const other=location.value==='other';$('#smartOtherWrap').classList.toggle('hide',!other)}location.onchange=sync;sync();
  ...
  async function generatePO(){
    const doc=smartState.doc;if(!doc)return;const out=$('#smartGenerateMsg'),btn=$('#smartGenerate'),location=$('#smartLocation').value,companyId=$('#smartCompany').value,destination=location==='other'?'other':'warehouse',warehouseId=destination==='warehouse'?location:null,destinationText=destination==='other'?$('#smartDestText').value.trim():null,bargeId=$('#smartBarge').value||null;
    if(!companyId)return msg(out,'Elegí la empresa compradora.');if(destination==='warehouse'&&!warehouseId)return msg(out,'Elegí la ubicación de entrega.');if(destination==='other'&&!destinationText)return msg(out,'Escribí la otra ubicación de entrega.');
  ...
  function companyLogo(c){const n=norm(`${c?.name||''} ${c?.legal_name||''}`);if(n.includes('astillero villa hayes'))return assetUrl('assets/logo-avh.jpg');if(n.includes('maqmoveis'))return assetUrl('assets/logo-maqmoveis.jpg');return''}
  ...
  function projectLabel(p){return p.barge_number?`Barcaza ${p.barge_number}`:(p.barge_id?`Barcaza ${bargeNo(p.barge_id)}`:'—')}

  function poHtml(p,items){
    const c=companyById(p.company_id)||{},s=supplierById(p.supplier_id)||{},logo=companyLogo(c),total=items.reduce((a,x)=>a+num(x.quantity)*num(x.unit_price),0),contactPhone=c.phone||'0971 800 829',contactEmail=c.email||'gortega@astillerovh.com',reference=p.source_document_number||p.order_reference||'—';
    return `<div class="po-sheet" id="poSheet"><div class="po-head"><div class="po-brand">${logo?`<img class="po-logo" src="${safe(logo)}" alt="${safe(c.name||'Empresa')}">`:''}<div class="po-company"><b>${safe(c.legal_name||c.name||p.company_name||'')}</b>${c.tax_id?`<div>RUC ${safe(c.tax_id)}</div>`:''}${c.address?`<div>${safe(c.address)}</div>`:''}${c.phone?`<div>${safe(c.phone)}</div>`:''}${c.email?`<div>${safe(c.email)}</div>`:''}</div></div><div style="text-align:right"><div class="po-title">ORDEN DE COMPRA</div><div class="po-no">${safe(p.po_number||'OC')}</div><div>Fecha: ${dateOnly(p.po_generated_at||p.created_at)}</div></div></div>
      <div class="po-grid"><div class="po-box"><span>PROVEEDOR</span><b>${safe(s.name||p.supplier_name||'—')}</b>${s.tax_id?`<div>RUC ${safe(s.tax_id)}</div>`:''}${s.phone?`<div>Tel. ${safe(s.phone)}</div>`:''}${s.email?`<div>${safe(s.email)}</div>`:''}</div><div class="po-box"><span>ENTREGA / DESTINO</span><b>${safe(destLabel(p)||'—')}</b>${p.barge_id?`<div>Proyecto: ${safe(projectLabel(p))}</div>`:''}<div>Contacto: Gabriel Ortega · Compras</div><div>${safe(contactPhone)}${contactEmail?' · '+safe(contactEmail):''}</div></div></div>
      <div class="po-meta-grid"><div class="po-meta"><span>REFERENCIA</span><b>Presupuesto ${safe(reference)}</b>${p.source_document_date?`<div>${dateOnly(p.source_document_date)}</div>`:''}</div><div class="po-meta"><span>CONDICIÓN DE PAGO</span><b>${safe(p.payment_terms||'Según presupuesto')}</b>${p.payment_method?`<div>${safe(p.payment_method)}</div>`:''}</div><div class="po-meta"><span>PLAZO / ENTREGA</span><b>${p.expected_date?dateOnly(p.expected_date):'Según presupuesto'}</b></div><div class="po-meta"><span>MONEDA</span><b>${safe(p.currency||'PYG')}</b></div></div>
      <table class="po-table"><thead><tr><th>Código</th><th>Descripción técnica</th><th>Cantidad</th><th>Unidad</th><th>Precio unit.</th><th>Total</th></tr></thead><tbody>${items.map(x=>{const pr=productById(x.product_id)||{};return`<tr><td>${safe(pr.sku||'—')}</td><td>${safe(x.description)}</td><td>${fmt(x.quantity)}</td><td>${safe(x.unit)}</td><td>${money(x.unit_price,p.currency)}</td><td>${money(num(x.quantity)*num(x.unit_price),p.currency)}</td></tr>`}).join('')}</tbody></table><div class="po-total">TOTAL OC: ${money(total,p.currency)}</div>
      <div class="po-reference"><b>Referencia documental:</b> Presupuesto / cotización ${safe(reference)}${p.source_document_date?' · '+dateOnly(p.source_document_date):''}${p.notes?`<div style="margin-top:6px"><b>Observaciones:</b> ${safe(p.notes)}</div>`:''}</div>
      <div class="po-footer"><div><b>Responsable de Compras</b>Gabriel Ortega</div><div style="text-align:right"><b>${safe(c.legal_name||c.name||'')}</b>${safe(contactPhone)}${contactEmail?' · '+safe(contactEmail):''}</div></div></div>`
  }
  ...
  function printPurchaseOrder(id){
    ...
    tab.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${safe(p.po_number||'Orden de compra')}</title><style>@page{size:A4;margin:12mm}*{box-sizing:border-box}body{font-family:Arial,sans-serif;color:#17231d;margin:0;font-size:12px}.po-sheet{width:100%;background:#fff}.po-head{display:flex;justify-content:space-between;gap:20px;border-bottom:2px solid #183c2c;padding-bottom:12px}.po-brand{display:flex;gap:12px;align-items:center}.po-logo{width:175px;max-height:75px;object-fit:contain;object-position:left center}.po-company{font-size:11px;line-height:1.4}.po-title{font-size:22px;font-weight:900}.po-no{font-size:18px;font-weight:900}.po-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:12px 0}.po-box{border:1px solid #ccc;padding:9px;line-height:1.4}.po-box span,.po-meta span{display:block;font-size:9px;color:#666;font-weight:700}.po-meta-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:7px;margin:10px 0}.po-meta{border:1px solid #ccc;padding:8px;min-height:48px}.po-table{width:100%;border-collapse:collapse;margin-top:10px}.po-table th,.po-table td{padding:7px 5px;border-bottom:1px solid #ddd;text-align:left;vertical-align:top}.po-table th{font-size:9px;text-transform:uppercase}.po-table th:nth-child(n+3),.po-table td:nth-child(n+3){text-align:right}.po-total{text-align:right;font-size:18px;font-weight:900;margin-top:12px}.po-reference{margin-top:10px;padding-top:9px;border-top:1px solid #ddd;font-size:10px}.po-footer{display:flex;justify-content:space-between;margin-top:24px;padding-top:10px;border-top:1px solid #ddd;font-size:10px}.po-footer b{display:block}.subtext,.hint{font-size:10px;color:#666}</style></head><body>${poHtml(p,items)}</body></html>`);tab.document.close();tab.focus();setTimeout(()=>tab.print(),350);
  }
