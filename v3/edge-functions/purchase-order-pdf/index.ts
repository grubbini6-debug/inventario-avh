...
const paymentText=(v:unknown)=>{const t=safe(v);if(!t)return 'Según presupuesto';if(/^cred\.?$/i.test(t))return 'Crédito';if(/^cont\.?$/i.test(t))return 'Contado';return t};
...
function drawHeader(page:PDFPage,company:any,bold:PDFFont,regular:PDFFont,po:string,date:string){
  const leftX=36,leftW=348,rightX=410;
  page.drawText('COMPRADOR',{x:leftX,y:814,font:bold,size:6.8,color:muted});
  let yy=798;
  yy=drawText(page,safe(company?.legal_name||company?.name||'Empresa compradora'),leftX,yy,bold,12.2,green,leftW);
  const details=[
    company?.tax_id?`RUC ${company.tax_id}`:'',
    company?.address||'',
    [company?.phone,company?.email].filter(Boolean).join(' · ')
  ].filter(Boolean);
  for(const detail of details)yy=drawText(page,detail,leftX,yy-1,regular,7.1,muted,leftW);
  page.drawText('ORDEN DE COMPRA',{x:rightX,y:801,font:bold,size:13.6,color:dark});
  page.drawText(po,{x:rightX,y:782,font:bold,size:10.8,color:green});
...
function pageBase(pdf:PDFDocument,company:any,bold:PDFFont,regular:PDFFont,po:string,date:string){const page=pdf.addPage([595.28,841.89]);drawHeader(page,company,bold,regular,po,date);return page}
