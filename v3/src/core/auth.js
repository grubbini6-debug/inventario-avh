...
async function signOut(){
  const old=session;saveSession(null);profile=null;clearInterval(refreshTimer);showApp(false);
  const pass=$('#loginPass');if(pass)pass.value='';
  try{
    if(old?.access_token)await fetch(`${API}/auth/v1/logout`,{method:'POST',headers:{apikey:KEY,Authorization:`Bearer ${old.access_token}`}});
  }catch{}
  location.reload();
}
