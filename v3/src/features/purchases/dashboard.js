...
(function(){
  ...
  function enhanceAlerts(){
    ...
    document.querySelectorAll('[data-smart-stock]').forEach(row=>{
      ...
      sub.innerHTML=`<b>${esc(x.risk_reason||'Revisar abastecimiento')}</b><br>${esc(x.warehouse_name)} · stock ${fmt(x.stock_qty)} ${esc(x.base_unit)} · consumo ${fmt(x.avg_daily_30d)} ${esc(x.base_unit)}/día · cobertura ${x.coverage_days==null?'—':fmt(x.coverage_days)+' días'}${inbound>0?`<br>En camino: <b>${fmt(inbound)} ${esc(x.base_unit)}</b>${x.next_expected_date?` · próxima recepción ${shortDate(x.next_expected_date)}`:''}${x.next_supplier_name?` · ${esc(x.next_supplier_name)}`:''}`:''}${recommended>0?`<br>Compra sugerida: <b>${fmt(recommended)} ${esc(x.base_unit)}</b>${x.preferred_supplier_name?` · preferido ${esc(x.preferred_supplier_name)}`:''}`:''}`;
