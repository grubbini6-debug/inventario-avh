...
(function(){
  ...
  function openCompleteDocs(purchaseId){
    ...
    $('#depDocSave').onclick=async()=>{const inv=$('#depDocInvoiceFile').files?.[0],rem=$('#depDocRemitFile').files?.[0];if(!inv&&!rem)return msg($('#depDocMsg'),'Elegí al menos un archivo.');const b=$('#depDocSave');b.disabled=true;b.textContent='Guardando…';try{if(inv){const saved=await registerReceiptDoc(receipt.id,purchaseId,'invoice',inv,$('#depDocInvoiceNo').value,$('#depDocInvoiceDate').value);if(saved?.documentId&&window.AVHPurchaseControl?.analyzeInvoice)try{await window.AVHPurchaseControl.analyzeInvoice(purchaseId,saved.documentId,{reload:false})}catch(e){console.warn('Factura guardada; análisis pendiente',e)}}if(rem)await registerReceiptDoc(receipt.id,purchaseId,'remittance',rem,$('#depDocRemitNo').value,$('#depDocRemitDate').value);await loadAll(true);closeModal()}catch(e){msg($('#depDocMsg'),e.message||String(e))}finally{b.disabled=false;b.textContent='Guardar documentos'}};
  }
