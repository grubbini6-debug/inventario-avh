// AVH — Presentación profesional de OC + descarga rápida desde PDF archivado.
(function(){
  ...
  const items=id=>(D.purchaseItems||[]).filter(x=>x.purchase_id===id);
  const company=id=>(D.purchaseCompanies||[]).find(x=>x.id===id)||{};
  ...
  function purchaseFromModal(){
    const direct=document.querySelector('#modalBody .po-no')?.textContent?.trim();if(direct){const p=purchaseByPo(direct);if(p)return p}
  ...
  function polishedHtml(p){
    ...
    const project=p.barge_number?`Barcaza ${p.barge_number}`:(p.barge_id&&typeof bargeNo==='function'?`Barcaza ${bargeNo(p.barge_id)}`:'—');
    return `<div class="po-top"><div class="po-company"><div class="po-logo-wrap"><img class="po-real-logo" src="${logoFor(p)}" alt="Logo"></div><div><div class="po-buyer-name">${safe(c.legal_name||c.name||p.company_name||'')}</div><div class="po-buyer-meta">${c.tax_id?`RUC ${safe(c.tax_id)}<br>`:''}${c.address?safe(c.address)+'<br>':''}${[c.phone,c.email].filter(Boolean).map(safe).join(' · ')}</div></div></div><div class="po-doc"><div class="po-doc-title">ORDEN DE COMPRA</div><div class="po-doc-number po-no">${safe(p.po_number||'OC')}</div><div class="po-doc-date">Fecha: ${dateOnly(p.po_generated_at||p.created_at)}</div></div></div>
      <div class="po-body"><div class="po-info-grid"><div class="po-info-card"><div class="po-info-title">PROVEEDOR</div><b>${safe(s.name||p.supplier_name||'Proveedor sin definir')}</b><div class="po-info-line">${s.tax_id?`RUC ${safe(s.tax_id)}`:'RUC no registrado'}${s.phone?` · Tel. ${safe(s.phone)}`:''}</div></div><div class="po-info-card"><div class="po-info-title">ENTREGA</div><b>${safe(dest(p)||'—')}</b><div class="po-info-line">${p.expected_date?`Fecha prevista: ${dateOnly(p.expected_date)}`:'Plazo: según presupuesto / coordinación'}</div></div></div>
      <div class="po-order-grid"><div class="po-kv"><span>Presupuesto / referencia</span><b>${safe(p.source_document_number||p.order_reference||'—')}</b></div><div class="po-kv"><span>Condición de pago</span><b>${safe(p.payment_terms||'Según presupuesto')}</b></div><div class="po-kv"><span>Moneda</span><b>${safe(p.currency||'PYG')}</b></div><div class="po-kv"><span>Proyecto</span><b>${safe(project)}</b></div></div>
      ...
      <div class="po-sum"><div class="po-total-box"><span>TOTAL ORDEN DE COMPRA</span><b>${money(total,p.currency)}</b></div></div><div class="po-foot"><b>Contacto de Compras:</b> Gabriel Ortega${c.phone?` · ${safe(c.phone)}`:' · 0971 800 829'}${c.email?` · ${safe(c.email)}`:' · gortega@astillerovh.com'}<br>Esta Orden de Compra corresponde al presupuesto/referencia indicado y queda registrada en AVH junto con su documentación y recepción.</div></div>`;
  }

  function polishCurrentModal(){
    const sheet=document.querySelector('#modalBody .po-sheet');if(!sheet||sheet.dataset.polished==='1')return;
    const p=purchaseFromModal();if(!p)return;sheet.classList.add('po-polished');sheet.dataset.polished='1';sheet.innerHTML=polishedHtml(p);
    const status=document.querySelector('#modalBody .po-status');if(status)status.textContent=(p.status==='ordered'||p.purchase_confirmed_at)?'COMPRADO / PEDIDO':'OC GENERADA · PENDIENTE DE CONFIRMAR COMPRA';
  }
  ...
  async function downloadStored(doc,p){
    const u=await signedUrl(doc.file_path),r=await fetch(u);if(!r.ok)throw Error('No pude descargar la OC archivada.');const blob=await r.blob(),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=doc.file_name||`${p.po_number||'Orden-de-Compra'}.pdf`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),2500);
  }
  ...
  document.addEventListener('click',e=>{
    const btn=e.target?.closest?.('#poDownload,#detailDownloadPo');if(!btn)return;const p=purchaseFromModal();if(!p)return;e.preventDefault();e.stopImmediatePropagation();fastDownload(p,btn).catch(err=>alert(err.message||String(err)));
  },true);
  const observer=new MutationObserver(()=>polishCurrentModal());observer.observe(document.body,{childList:true,subtree:true});
  document.addEventListener('click',()=>setTimeout(polishCurrentModal,0),true);
  window.AVHPOPolish={polishCurrentModal,fastDownload};
})();
