...
if (inlineScripts.length < 5) throw new Error('La base legacy no contiene los bloques esperados.');
...
// Los bloques legacy 0..3 ya fueron sustituidos por módulos legibles.
const remainingLegacyCore = inlineScripts.slice(4).map((code,i)=>`\n/* ===== legacy core block ${i+4} ===== */\n${code}`).join('\n');
...
console.log(`AVH V3 build OK: ${manifest.core.length} módulos base + ${inlineScripts.length-4} bloque(s) legacy + ${manifest.features.length} extensiones`);
