...
Deno.serve(async(req:Request)=>{
  ...
  try{
    const body=await req.json(),mode=String(body.mode||'');if(mode!=='receipt')throw Error('Esta IA está habilitada únicamente para recepción de órdenes de compra.');
    ...
    if(!warehouseId)throw Error('No se pudo determinar el depósito de recepción.');
    const context:any={warehouse_id:warehouseId};
    ...
    // Defensa: ids devueltos deben pertenecer a la OC pendiente elegida del depósito.
    const selected=(context.pending_purchases||[]).find((p:any)=>p.purchase_id===analysis.matched_purchase_id);
    if(!selected){
      analysis.matched_purchase_id=null;analysis.matched_po_number=null;analysis.matched_supplier_name=null;
      for(const it of analysis.detected_items||[]){it.purchase_item_id=null;it.product_id=null}
    }else{
      const validItems=new Map(selected.items.map((x:any)=>[x.purchase_item_id,x]));
      analysis.matched_po_number=selected.po_number;analysis.matched_supplier_name=selected.supplier;
      for(const it of analysis.detected_items||[]){
        const src=validItems.get(it.purchase_item_id);
        if(!src){it.purchase_item_id=null;it.product_id=null}else it.product_id=src.product_id;
      }
    }
